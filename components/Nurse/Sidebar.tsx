...
export default function Sidebar({ onCollapseChange }: { onCollapseChange: (collapsed: boolean) => void }) {
  ...
  const sideBarLinks = [
    { name: 'Home', path: '/dashboard-nurse', icon: BiHomeAlt },
    { name: 'Forums', path: '/Forums-nurse', icon: HiUsers },
    { name: 'Sessions', path: '/Sessions-nurse', icon: HiOutlineChatAlt2 },
    { name: 'E.H.R', path: '/ehr-nurse', icon: HiOutlineCalendar },
  ];
  ...
  return (
    <div
      className={`h-screen transition-width duration-300 ${isCollapsed ? 'w-16' : 'w-48'} bg-[#0b2047] text-white flex flex-col fixed z-10`}
